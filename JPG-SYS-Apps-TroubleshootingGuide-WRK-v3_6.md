...
**Document ID:** JPG-SYS-Apps-TroubleshootingGuide-WRK-v3.6

**Date:** 07/10/2026 | **Prepared by:** Claude | **State:** WRK

**Classification:** CLASS 1 — CONFIDENTIAL

**Supersedes:** JPG-SYS-Apps-TroubleshootingGuide-WRK-v3.5
...
**v3.6 update (07/10/2026):** The PIT To Accomplish + Appointments
carryover system is now built and verified (07/09–07/10/2026 build
session). Section 2L moved from design-locked to built. Section 2C
(To Accomplish), Section 2D (Archive), and Section 2E (Appointments
and Future Tasks) rewritten to built behavior. Future Tasks Remove
button gap closed. Appointments no-default-empty-box behavior added.
...
## **SECTION 2C — TO ACCOMPLISH (Q2 / Q3 SECTIONS) (UPDATED 07/10/2026)**

**Unfinished To Accomplish items now carry forward to the next day
automatically.** An item the client does not check off stays visible
the next day, and the day after, until it is checked done or cleared.
It no longer disappears overnight.

**Checking an item off now records it as done on the day it started,
not the day it was checked.** The item is marked done on its original
day and today's slot opens up for something new.

**A checked-off item now leaves today's slot empty and does not come
back the next day.** Once it is done, it stays done and no longer
carries.

**Carried items now keep their original position.** The top priority
stays the top priority; a second-position item stays in the second
position from one day to the next.

**The client can clear items directly from the To Accomplish section.**
Full carryover and clearing behavior is described in Section 2L.

## **SECTION 2D — ARCHIVE (UPDATED 07/10/2026)**
...
**The Archive is now view-only.** The client can look back at any
past day but cannot check, uncheck, clear, add, or remove anything
on a past day. Past days are a record, not an editing surface.

**Days with no PIT entry now show "No PIT entry for this day."** A
day the client skipped entirely no longer opens a blank form — it
clearly states there was no entry.

## **SECTION 2E — APPOINTMENTS AND FUTURE TASKS (UPDATED 07/10/2026)**

**Appointments no longer starts with an empty box.** The section
shows only the "Add Appointment" button until the client adds one,
and it returns to that clean state when the last appointment is
removed.

**Past-due appointments now stay on the list with a red "Past Due"
badge until the client acts on them.** An appointment whose date has
passed no longer disappears — it follows the client, clearly marked,
until it is checked off or removed.

**Checking an appointment off now marks it done and takes it off the
list.** A checked appointment is recorded as done on its scheduled
date and no longer shows.

Appointments' existing "Remove" button still works — a direct delete
with no record kept.

**Future Tasks now has a Remove button on every slot, including the
last one.** The client can clear future tasks one at a time all the
way down to none, and add them back with the "Add Future Task" button
when needed. This closes the earlier confirmed gap.
...
A significant combined copy pass is still held, covering: Q2/Q3
carryover wording, "Daily Trackables" sub-heading, panel-wide layout
restructure, auto-save reminder text, and AI Summary description
update. Do not send pieces individually.

**The Q2/Q3 carryover wording is now ready to be rewritten.** The
old "no carryover" language is out of date now that the carryover
behavior (Sections 2C and 2L) is built. The rewrite still waits for
the combined copy pass to be sent as one piece.
...
## **SECTION 2L — TO ACCOMPLISH + APPOINTMENTS CARRYOVER (BUILT 07/10/2026)**

This section describes the carryover behavior of the To Accomplish
and Appointments sections. It was design-locked 07/08/2026 and built
and verified across the 07/09–07/10/2026 session. The full technical
design lives in the PIT Code Logic doc, Section H.

### **2L.1 — Carrying forward**

**Unchecked To Accomplish items now carry forward to the next day
automatically.** An item added today stays visible tomorrow, and the
day after, until the client either checks it done or clears it.

**Checking an item off now records it as done on the day it was
originally added — not the day it was checked.** The same origin-day
rule applies to clearing an item.

**Appointments now follow the same carryover pattern.** An
appointment scheduled for a past date keeps showing up with a
visible "Past Due" badge until the client acts on it — no
auto-expiry.

### **2L.2 — Fixed slots**

To Accomplish is not a growing list. It has fixed positions — the
One Thing at the top, then the following items in order. **Carried
items now stay in the same slot position they had the day before.**

### **2L.3 — Clearing items selectively**

**A "Clear Items" button now appears in the To Accomplish section.**
The button only shows when at least one slot has an item in it, and
sits at the top-right of the section.

**Clicking the button now opens a small window listing each occupied
slot with a checkbox.** The client picks which items to clear and
confirms. Cleared items are removed from today's slots and recorded
as "cleared" on the day they were added.

**A Cancel option is always available in that window.** Confirming
shows a brief notification — "1 item cleared" for a single item, or
"N items cleared" for more than one — that fades on its own.

### **2L.4 — Archive behavior**

**The Archive is now read-only** (see Section 2D). The client can
view past days but cannot check, uncheck, clear, or edit any item
from a past day.

**Days the client skipped PIT entirely now show "No PIT entry for
this day"** in the Archive instead of a blank empty form.

### **2L.5 — Instructions panel implications**

The current Q2/Q3 copy in the instructions panel still explains the
old "no carryover" behavior and is now out of date. The rewrite is
ready but still waits for the held combined instructions-panel copy
pass to be sent as one piece (see Section 2J).
...
- Whether the Archive shows a per-day "carried, unresolved" marker
  for a lingering item — the carryover data supports it, but no such
  marker is displayed in the Archive yet (future display decision).
...
| v3.6 | 07/10/2026 | PIT To Accomplish + Appointments carryover system BUILT and verified (07/09–07/10/2026 session). Section 2L moved from design-locked to built and rewritten to declarative built statements (carry forward, origin-day check-off/clear, past-due appointments, fixed slots, "Clear Items" button + modal + notification, read-only Archive, "No PIT entry for this day"). Section 2C (To Accomplish) rewritten to built carryover behavior. Section 2D (Archive) updated: now view-only + no-entry-day message. Section 2E (Appointments and Future Tasks) rewritten: Future Tasks Remove button gap closed (works on every slot including the last), Appointments no default empty box, past-due badge, appointment check-off. Section 2J updated: Q2/Q3 carryover copy rewrite now ready, still held for the combined pass. "Clear Items" final button wording recorded. One flagged gap added: Archive does not yet display a per-day "carried, unresolved" marker (data supports it; display decision deferred). |

*JPG-SYS-Apps-TroubleshootingGuide-WRK-v3.6 | Jones Performance
Group LLC | CONFIDENTIAL | 07/10/2026*
