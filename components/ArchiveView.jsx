...
export default function ArchiveView({ archive, weekData, completedBooks, streak, currentUser, setCU, setView, openArchive, dayCompleteDates = [], backToday }) {
  return (
      ...
      <div style={{ background: '#111', borderBottom: `2px solid ${GOLD}`, padding: '0 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', height: 58 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ display: 'flex', gap: 8 }}>
            <button style={gbtn({ background: '#333', fontSize: 12, color: '#fff', border: 'none' })} onClick={backToday}>Today</button>
